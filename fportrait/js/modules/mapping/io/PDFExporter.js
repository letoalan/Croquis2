...
export class PDFExporter {
    ...
    async _captureFullMap(map, mapContainer) {
        ...
        // 2. Capture isolée de l'échelle et de la rose des vents AVANT masquage des contrôles UI
        let scaleCanvas = null;
        let orientationCanvas = null;
        let scalePos = 'topright';
        let orientationPos = 'topright';
        ...
        try {
            ...
            if (scaleContainer && this._hasVisibleSize(scaleContainer)) {
                scaleCanvas = await html2canvas(scaleContainer, { useCORS: true, allowTaint: true, logging: false, scale: 2, backgroundColor: null });
            }
            if (orientationContainer && this._hasVisibleSize(orientationContainer)) {
                orientationCanvas = await html2canvas(orientationContainer, { useCORS: true, allowTaint: true, logging: false, scale: 2, backgroundColor: null });
            }
        } catch (err) {
        ...
        try {
            ...
            // 4. Capturer les colonnes de légende
            const legendData = await LegendPdfRenderer.captureLegend(legendContainer, scale);
            ...
            let orientationBottom = titleHeight + margin;
            ...
            if (orientationCanvas) {
                ...
                orientationBottom = oy + orientationCanvas.height;
            }
            ...
            if (scaleCanvas) {
                ...
                if (scalePos.includes('top')) {
                    // Si même coin que l'orientation (ex: topright dans fportrait), empiler sous l'orientation
                    sy = (orientationCanvas && scalePos === orientationPos) ? orientationBottom + (10 * scale) : (titleHeight + margin);
                }
