...
export class StateManager {
    ...
    /**
     * ✅ OUVRE L'ÉDITEUR POUR UNE GÉOMÉTRIE (MODIFIÉ POUR PROMETHEAN)
     */
    openContextMenu(index) {
        if (index === null || index === undefined || !this.geometries[index]) {
            console.error('[StateManager] ❌ Invalid index for openContextMenu');
            return;
        }

        const geometry = this.geometries[index];
        this.selectedIndex = index;
        console.log('[StateManager] 🎨 Opening editor for geometry:', index, geometry.name);

        // ✅ 1. Activer le mode édition PM (points oranges) sur la couche
        if (geometry.layer && geometry.layer.pm) {
            console.log('[StateManager] 🏗️ Activation de pm.enable() sur le layer');
        ...
        // ✅ 2. Remplir les champs de l'éditeur dans le panneau de droite
        if (this.uiManager) {
            this.uiManager.populateContextMenuForGeometry(geometry);
        }

        // ✅ 3. S'assurer que le panneau "Figurés" est activé (si pas déjà fait)
        const panelDraw = document.getElementById('panel-draw');
        if (panelDraw && !panelDraw.classList.contains('active')) {
            console.log('[StateManager] 📱 Forcing Figurés panel activation');
            // On simule un clic sur l'icône Figurés ou on utilise UIManager si disponible
            if (this.uiManager && this.uiManager.setActivePanel) {
                this.uiManager.setActivePanel('draw');
            }
        }
    }
    ...
    applyStyle(color, lineColor, opacity, lineDash, lineWeight, markerSize) {
        ...
        if (this.selectedIndex === null) {
            // ✅ Auto-sélection : si une seule géométrie existe, la sélectionner automatiquement
            if (this.geometries.length === 1) {
                console.log('[StateManager] 🔄 Auto-selecting the only geometry (index 0)');
                this.selectedIndex = 0;
            } else if (this.geometries.length > 1) {
                console.warn('[StateManager] ⚠️ No geometry selected — please click on a geometry in the list first');
                return;
            } else {
                console.warn('[StateManager] ⚠️ No geometry to apply style to');
                return;
            }
        }
    ...
    updateGeometryList() {
        ...
        this.geometries.forEach((geometry, index) => {
            ...
            // ✅ Clic sur la ligne entière → sélectionner la géométrie et remplir l'éditeur
            listItem.addEventListener('click', (e) => {
                ...
                document.querySelectorAll('#geometryList .list-item').forEach(el => {
                    el.classList.remove('selected');
                });
                listItem.classList.add('selected');

                this.selectGeometry(index);
                console.log('[StateManager] List item clicked → selectGeometry:', index);
            });
        ...
        // ✅ Si une géométrie était sélectionnée, restaurer la surbrillance
        if (this.selectedIndex !== null) {
            const items = document.querySelectorAll('#geometryList .list-item');
    ...
    finishEditingGeometry(index) {
        if (index === null || index === undefined || !this.geometries[index]) return;
        const geometry = this.geometries[index];
        if (!geometry) return;

        console.log('[StateManager] 🏁 Finishing editing for geometry:', index);

        // ✅ Désactiver le mode édition PM
        if (geometry.layer && geometry.layer.pm) {
            geometry.layer.pm.disable();
        }

        // ✅ Restaurer l'icône si c'était un CustomMarker édité temporairement
        if (this.temporarySVGs && this.temporarySVGs.has(index)) {
