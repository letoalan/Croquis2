...
export class LegendManager {
     ...
     */
    initLegend() {
        console.log('[LegendManager] Initializing floating legend...');
        
        // On n'utilise plus L.Control pour Promethean, mais le conteneur HTML fixe
        this.container = document.getElementById('legendContainer');
        
        // Les contrôles de collapse sont maintenant gérés par la navigation du rail droit

        // ✅ Créer la première partie par défaut à l'initialisation
        ...
        console.log('[LegendManager] Floating legend initialized');
    }
    ...
    updateLegend() {
        ...
        // ✅ Pour Promethean, on ne réinjecte pas le header (il est dans le HTML)
        // On vide seulement le contenu pour le régénérer
        container.innerHTML = '';

        // ✅ Listener pour le bouton + Partie (dans le document)
        const addPartBtn = document.getElementById('addPartBtn');
        if (addPartBtn && !addPartBtn._hasListener) {
            addPartBtn.addEventListener('click', () => this._addNewPart());
            addPartBtn._hasListener = true;
        }
    ...
    _addNewPart() {
        ...
        const defaultTitle = romanNumerals[partNumber - 1] || `Partie ${partNumber}`;

        const partName = prompt('Nom de la nouvelle partie:', defaultTitle);
        if (partName !== null && partName.trim()) {
            this.stateManager.addLegendPart(partName.trim());
            console.log('[LegendManager] New part added:', partName.trim());
            this.updateLegend();
        }
    }
