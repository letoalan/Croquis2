...
export class SymbolPaletteManager {
    ...
    initialize() {
        ...
        // ✅ ATTENDRE QUE LE DOM SOIT COMPLÈTEMENT PRÊT AVANT D'ACCÉDER AUX ÉLÉMENTS
        const waitForDOM = () => {
            ...
            if (textEditor && dropZonesContainer && storageContainer) {
                console.log('[SymbolPaletteManager] ✅ DOM elements found, continuing initialization...');
                this.doInitialize();
            } else {
                const missing = [];
                if (!textEditor) missing.push('textEditor');
                if (!dropZonesContainer) missing.push('dropZonesContainer');
                if (!storageContainer) missing.push('usedSymbolsStorage');
                console.warn(`[SymbolPaletteManager] ⏳ Waiting for DOM elements: ${missing.join(', ')}`);
    ...
    doInitialize() {
        ...
        // ✅ INITIALISATION APRÈS QUE LE DOM SOIT PRÊT
        setTimeout(() => {
    ...
    syncDropZoneLines() {
        const textEditor = document.getElementById('textEditor');
        const dropZonesContainer = document.getElementById('dropZonesContainer');
        const synchronizedContainer = document.querySelector('.editor-work-area');

        if (!textEditor || !dropZonesContainer) {
            console.warn('[SymbolPaletteManager] ⚠️ Required containers not found');
            return;
        }

        const lineHeight = 40; // Augmenté pour Promethean (au lieu de 28)
    ...
    initializeSynchronizedScrolls() {
        ...
        synchronizedContainer.addEventListener('scroll', () => {
            ...
            isScrolling = true;
            syncScrollPosition(synchronizedContainer.scrollTop);
            setTimeout(() => {
                isScrolling = false;
            }, 10);
        });
    ...
    initializeTextEditorPlaceholder() {
        ...
        const syncPlaceholderState = () => {
            const empty = textEditor.textContent.trim() === '';
            textEditor.dataset.empty = empty ? 'true' : 'false';
        };

        textEditor.addEventListener('focus', syncPlaceholderState);
        textEditor.addEventListener('paste', () => setTimeout(syncPlaceholderState, 0));
        textEditor.addEventListener('input', syncPlaceholderState);
        textEditor.addEventListener('blur', syncPlaceholderState);
